        ...
        if(group!==null&&typeof group!=='undefined'&& group.length>0)
            {
              ...
              { intermediate1=this.dealWithApply(apply,intermediate)}
                else
                { for(var intermediateObj of intermediate)
                intermediate1.push(intermediateObj[0])
                }
        }
        else{intermediate1=intermediate}

        var finalResultObjArray: any = this.represent(get, intermediate1)
...
// Check to see if GET is string or Array
    ...
    public checkGroupCorrect(group:any,intermediate:any,lastintermediates:any):boolean{
        ...
        for (var a=0;a< group.length;a++)
        {
            if(intermediate[group[a]]===lastintermediates[a])
            {validlist.push(true);}
            else
